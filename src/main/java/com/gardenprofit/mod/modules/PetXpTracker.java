...
public class PetXpTracker {
    ...
    public static long[] getXpTable(GardenProfitConfig.PetRarity rarity, int maxLevel) {
        long[] base;
        switch (rarity) {
            case COMMON:
    ...
    public static void update(Minecraft client) {
        ...
        GardenProfitConfig.PetInfo activePet = null;
        int detectedLevel = -1;
        int petLineIndex = -1;
        ...
        if (previousXp >= 0 && absoluteXp > previousXp) {
            ...
            if (delta < 2_000_000L) {
                ProfitManager.getInstance().addPetXp(activePet.name, delta);
            }
        }
